...
build-linux-amd64: | $(BIN_DIR)
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 $(GO) build -o $(BIN_DIR)/orvix-linux-amd64 .

build-linux-arm64: | $(BIN_DIR)
	GOOS=linux GOARCH=arm64 CGO_ENABLED=0 $(GO) build -o $(BIN_DIR)/orvix-linux-arm64 .

build-darwin-amd64: | $(BIN_DIR)
	GOOS=darwin GOARCH=amd64 CGO_ENABLED=0 $(GO) build -o $(BIN_DIR)/orvix-darwin-amd64 .

build-darwin-arm64: | $(BIN_DIR)
	GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 $(GO) build -o $(BIN_DIR)/orvix-darwin-arm64 .

build-windows-amd64: | $(BIN_DIR)
	GOOS=windows GOARCH=amd64 CGO_ENABLED=0 $(GO) build -o $(BIN_DIR)/orvix-windows-amd64.exe .
